...
import { generateStockReportChamberWise, getUniqueChamberNames } from '@/lib/services/stock-report.service';
import type { Client } from '@/lib/types';
import type { StockReportChamberWiseData, StockReportChamberGroup, StockReportChamberRow } from '@/lib/types/stock-report';
import { format } from 'date-fns';
import { PrintHeader } from '@/components/print/PrintHeader';

export default function StockReportChamberWisePage() {
  ...
  const [chamberNames, setChamberNames] = useState<string[]>([]);
  const [selectedChamber, setSelectedChamber] = useState<string | null>(null);
  const [chamberInput, setChamberInput] = useState<string>('');
  const [filteredChambers, setFilteredChambers] = useState<string[]>([]);
  const [showChamberSuggestions, setShowChamberSuggestions] = useState(false);
  const [chamberHighlightIndex, setChamberHighlightIndex] = useState(0);

  const [asOnDate, setAsOnDate] = useState<string>(format(new Date(), 'yyyy-MM-dd'));

  const [reportData, setReportData] = useState<StockReportChamberWiseData | null>(null);
  ...
  // Load chamber names on mount and when client changes
  useEffect(() => {
    const loadChamberNames = async () => {
      try {
        const names = await getUniqueChamberNames(selectedClient?.id);
        setChamberNames(names);
        // Reset chamber selection when client changes
        setSelectedChamber(null);
        setChamberInput('');
        setFilteredChambers([]);
        setShowChamberSuggestions(false);
      } catch (error) {
        toast({ title: 'Error', description: 'Failed to load chamber names.', variant: 'destructive' });
      }
    };
    loadChamberNames();
  ...
  const handleChamberInputChange = useCallback((value: string) => {
    setChamberInput(value);
    setSelectedChamber(null);

    if (!value.trim()) {
      setFilteredChambers([]);
      setShowChamberSuggestions(false);
      setChamberHighlightIndex(0);
      return;
    }

    const results = chamberNames.filter((c) => c.toLowerCase().includes(value.toLowerCase()));
    setFilteredChambers(results);
  ...
  const handleChamberKeyDown = useCallback((e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'ArrowDown') {
      if (!showChamberSuggestions || filteredChambers.length === 0) return;
      e.preventDefault();
      setChamberHighlightIndex((prev) => (prev + 1) % filteredChambers.length);
      return;
    }
    ...
    if (e.key === 'ArrowUp') {
      if (!showChamberSuggestions || filteredChambers.length === 0) return;
      e.preventDefault();
      setChamberHighlightIndex((prev) => (prev === 0 ? filteredChambers.length - 1 : prev - 1));
      return;
    }
    ...
    if (e.key === 'Enter') {
      e.preventDefault();
      if (showChamberSuggestions && filteredChambers.length > 0) {
        const selected = filteredChambers[chamberHighlightIndex];
        if (selected) handleChamberSelect(selected);
      }
    }
  }, [filteredChambers, handleChamberSelect, chamberHighlightIndex, showChamberSuggestions]);

  const handleShowReport = async () => {
    setLoading(true);
    try {
      const data = await generateStockReportChamberWise({
        customerId: selectedClient?.id,
        chamberName: selectedChamber || undefined,
        asOnDate: asOnDate,
      });
      setReportData(data);

      if (data.chamberGroups.length === 0) {
        toast({
          title: 'No Data',
  ...
  const calculateGrandTotal = () => {
    if (!reportData || reportData.chamberGroups.length === 0) return null;

    return reportData.chamberGroups.reduce(
      (acc, group) => ({
  ...
  const shouldShowGrandTotal = () => {
    return !selectedClient && !selectedChamber;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="w-full no-print">
        <CardHeader>
          <CardTitle>Stock Report Chamber Wise</CardTitle>
          <CardDescription>
            Select filters to generate chamber-wise stock report.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          {/* Chamber Selection */}
          <div className="chamber-selection space-y-2">
            ...
            <div className="relative">
              <Input
                ...
                type="text"
                value={chamberInput}
                onChange={(e) => handleChamberInputChange(e.target.value)}
                onKeyDown={handleChamberKeyDown}
                placeholder="Search chamber (optional - leave blank for all chambers)"
                className="w-full"
              />
              {showChamberSuggestions && filteredChambers.length > 0 && (
                <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-auto">
                  {filteredChambers.map((chamber, index) => (
                    <div
                      key={chamber}
                      className={`px-4 py-2 cursor-pointer ${
                        index === chamberHighlightIndex ? 'bg-sky-100' : 'hover:bg-gray-100'
                      }`}
      ...
      {reportData && reportData.chamberGroups.length > 0 && (
        <Card className="w-full mt-8 no-print">
          <CardContent className="p-0">
            <div className="text-center mb-4">
              <p className="text-sm font-semibold">STOCK REPORT CHAMBER WISE AS ON DATE : {formatDate(reportData.asOnDate)}</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-black text-xs" style={{ tableLayout: 'fixed' }}>
                ...
                <tbody>
                  {reportData.chamberGroups.map((group: StockReportChamberGroup, groupIndex: number) => (
                    <React.Fragment key={group.chamberName}>
                      {/* Chamber Header Row */}
                      ...
                      {/* Data Rows */}
                      {group.rows.map((row: StockReportChamberRow, rowIndex: number) => (
      ...
      {reportData && reportData.chamberGroups.length > 0 && (
        <div className="mt-8 print-only hidden">
          {/* Report Header */}
          <PrintHeader
            documentTitle="STOCK REPORT CHAMBER WISE"
          ...
          <table className="w-full border-collapse border border-black text-[10px]" style={{ tableLayout: 'fixed' }}>
            ...
            <tbody>
              {reportData.chamberGroups.map((group: StockReportChamberGroup, groupIndex: number) => (
                <React.Fragment key={group.chamberName}>
                  {/* Chamber Header Row */}
                  <tr className="bg-slate-100 font-bold chamber-header-row">
                    <td colSpan={11} className="border border-black px-2 py-2 text-center uppercase tracking-wide">
                      {group.chamberName}
                    </td>
                  </tr>
                  ...
                  <tr className="bg-slate-200 font-bold total-row">
                    <td colSpan={5} className="border border-black px-1 py-1 text-right">TOTAL</td>
              ...
              {shouldShowGrandTotal() && (() => {
                ...
                return (
                  <tr className="bg-slate-300 font-bold grand-total-row">
                    <td colSpan={5} className="border border-black px-1 py-1 text-right">GRAND TOTAL</td>
      ...
      <style jsx global>{`
        @media print {
          ...
          /* Avoid breaking chamber header from first row */
          .chamber-header-row {
