...
import { generateInwardOutwardWiseStockReport, getUniqueItemNames } from '@/lib/services/stock-report.service';
import type { Client } from '@/lib/types';
import type { StockReportInwardOutwardWiseData, StockReportInwardOutwardWiseRow } from '@/lib/types/stock-report';
import { PageHeader } from '@/components/page-header';
import { useRouter } from 'next/navigation';
import { format } from 'date-fns';

export default function StockReportInwardOutwardWisePage() {
  ...
  // Load item names when client changes
  useEffect(() => {
  ...
  const handleGenerateReport = async () => {
    ...
    if (!fromDate || !toDate) {
      toast({ title: 'Validation Error', description: 'From Date and To Date are required.', variant: 'destructive' });
      return;
    }
  ...
  const handlePrint = () => {
    ...
    router.push(`/printing/stock-report/inward-outward-wise/print?${params.toString()}`);
  };

  return (
    <div className="space-y-6">
      <PageHeader
        title="Inward / Outward Wise Report"
        description="View inward-wise outward movement and running stock balance"
      />
      ...
      <Card className="max-w-4xl mx-auto border shadow-sm">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2 justify-end">
            ...
            <Button onClick={handleGenerateReport} disabled={loading || !selectedClient || !fromDate || !toDate}>
        ...
        <Card className="max-w-6xl mx-auto border shadow-sm">
          <CardHeader className="py-4 flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>
                {reportData.customerName} | {format(new Date(reportData.fromDate), 'dd.MM.yyyy')} to {format(new Date(reportData.toDate), 'dd.MM.yyyy')}
              </CardDescription>
            </div>
          ...
          <CardContent>
            ...
            <div className="mb-6 p-4 bg-muted rounded-lg">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  <span className="font-semibold">Date Range:</span> {format(new Date(reportData.fromDate), 'dd.MM.yyyy')} - {format(new Date(reportData.toDate), 'dd.MM.yyyy')}
                </div>
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="border-b bg-muted">
                    <th className="px-3 py-2 text-left font-semibold">INW #</th>
                    <th className="px-3 py-2 text-left font-semibold">INW DATE</th>
                    ...
                    <th className="px-3 py-2 text-right font-semibold">INW QTY</th>
                    <th className="px-3 py-2 text-right font-semibold">INW WT</th>
                    ...
                    <th className="px-3 py-2 text-right font-semibold">OUT WT</th>
                    <th className="px-3 py-2 text-right font-semibold">BAL QTY</th>
                    <th className="px-3 py-2 text-right font-semibold">BAL WT</th>
                  </tr>
                </thead>
                <tbody>
                  {(() => {
                    ...
                    return reportData.rows.map((row, index) => {
                      ...
                      return (
                        ...
                        >
                          <td className="px-3 py-2 font-mono">{isFirstRowOfItem ? row.inwardNo : ''}</td>
                          <td className="px-3 py-2">{isFirstRowOfItem ? row.inwardDate : ''}</td>
                          <td className="px-3 py-2 uppercase">{isFirstRowOfItem ? row.itemName : ''}</td>
                          <td className="px-3 py-2">{isFirstRowOfItem ? row.brand : ''}</td>
                          <td className="px-3 py-2 text-right">{row.inwardQty !== undefined ? row.inwardQty : ''}</td>
                          <td className="px-3 py-2 text-right">{row.inwardWeight !== undefined ? row.inwardWeight.toFixed(2) : ''}</td>
                          <td className="px-3 py-2 font-mono">{row.outwardNo ?? ''}</td>
                          <td className="px-3 py-2">{row.outwardDate ?? ''}</td>
                          <td className="px-3 py-2 text-right">{row.outwardQty !== undefined ? row.outwardQty : ''}</td>
                          <td className="px-3 py-2 text-right">{row.outwardWeight !== undefined ? row.outwardWeight.toFixed(2) : ''}</td>
