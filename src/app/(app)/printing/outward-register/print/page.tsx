...
export default function OutwardRegisterPrintPage() {
  ...
  const rows: RegisterRow[] = useMemo(() => {
    return filteredVouchers.flatMap((voucher) => {
      return voucher.items.map((item) => {
        const outQty = parseNumberOrZero(item.bags);
        const outWeight = parseNumberOrZero(item.totalWeight);
  ...
  const totals = useMemo(() => {
    const totalQty = rows.reduce((sum, r) => sum + r.outQty, 0);
    const totalWeight = rows.reduce((sum, r) => sum + r.outWeight, 0);
    ...
    <div className="bg-slate-50 min-h-screen p-4 sm:p-8 print:bg-white print:p-0">
      <div className="max-w-6xl mx-auto bg-white p-8 border shadow-sm print:shadow-none print:border-none">
        <div className="text-center border-b-2 border-slate-800 pb-4 mb-6">
          ...
          {selectedClient ? <div className="mt-1 text-xs">CUSTOMER : {selectedClient.name}</div> : null}
        </div>
            ...
            <table className="w-full text-[11px] border-collapse">
              ...
              <tbody>
                ...
                <tr className="border-t-2 border-slate-800 bg-slate-50 font-bold">
                  <td colSpan={6} className="border-r border-slate-800 p-2 text-right uppercase">
                    Total
                  </td>
