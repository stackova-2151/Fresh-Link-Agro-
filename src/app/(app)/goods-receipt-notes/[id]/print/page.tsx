...
export default function PrintGRNPage() {
    ...
    useEffect(() => {
        if (params.id) {
            const found = goodsReceiptNotes.find(n => n.id === params.id);
            setNote(found || null);
        }
    }, [params.id]);

    if (!note) return <div className="p-8 text-center">Loading Goods Receipt Note...</div>;
        ...
        <div className="bg-slate-50 min-h-screen p-4 sm:p-8 print:bg-white print:p-0">
            <div className="max-w-4xl mx-auto bg-white p-8 border shadow-sm print:shadow-none print:border-none">
                ...
                <div className="text-center mb-6">
                    <span className="border-2 border-slate-800 px-10 py-1 font-bold text-sm bg-slate-50 uppercase tracking-widest">Goods Receipt Note</span>
                </div>
                ...
                <div className="grid grid-cols-12 gap-y-4 text-xs mb-8">
                    <div className="col-span-7 space-y-2 pr-8">
                        <div className="flex gap-2">
                            ...
                            <span className="border-b border-slate-300 flex-1 uppercase">{client?.name}</span>
                        </div>
                        <div className="flex gap-2">
                    ...
                    <div className="col-span-5 space-y-2 pl-4">
                        ...
                        <div className="flex gap-2">
                            ...
                            <span className="border-b border-slate-300 flex-1">{format(note.date, 'dd.MM.yyyy')}</span>
                        </div>
                        <div className="flex gap-2">
                    ...
                    <table className="w-full text-[11px] border-collapse">
                        ...
                        <tbody>
                            ...
                            <tr className="border-t-2 border-slate-800 bg-slate-50 font-bold h-10">
                                ...
                                <td className="border-r border-slate-800 p-2 text-center">{note.items.reduce((acc, i) => acc + i.quantity, 0)}</td>
                                <td className="p-2 text-right">{note.items.reduce((acc, i) => acc + i.weight, 0).toFixed(2)}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
