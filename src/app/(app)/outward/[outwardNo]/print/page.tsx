  ...
  return (
    <div className="bg-slate-50 min-h-screen p-4 sm:p-8 print:bg-white print:p-0">
      <div className="max-w-4xl mx-auto bg-white p-8 border shadow-sm print:shadow-none print:border-none">
        ...
        <div className="grid grid-cols-12 gap-y-4 text-xs mb-8">
          <div className="col-span-7 space-y-2 pr-8">
            ...
            <div className="flex gap-2"><span className="font-bold w-28 shrink-0">Remark :</span><span className="border-b border-slate-300 flex-1">{voucher.notes || 'N/A'}</span></div>
            <div className="flex gap-2"><span className="font-bold w-28 shrink-0">Driver Name :</span><span className="border-b border-slate-300 flex-1 uppercase">{voucher.driverName}</span></div>
          </div>
          <div className="col-span-5 space-y-2 pl-4">
            <div className="flex gap-2"><span className="font-bold w-24 shrink-0">Out No :</span><span className="border-b border-slate-300 flex-1 font-mono font-bold text-sm">{voucher.outwardNo}</span></div>
            ...
            <tbody>
              ...
              {[...Array(Math.max(0, 5 - voucher.items.length))].map((_, i) => (
                <tr key={`empty-${i}`} className="border-b border-slate-200 h-10">
                  ...
                  <td className="border-r border-slate-800" /><td className="border-r border-slate-800" />
                  <td className="border-r border-slate-800" /><td className="p-2" />
                </tr>
              ))}
        ...
        <div className="flex justify-between mt-12 text-xs">
          <div className="text-center w-40"><div className="pt-8 border-t border-slate-400"><p className="font-bold text-[10px]">Driver Signatory</p></div></div>
          <div className="text-center space-y-12">
            <p className="font-bold italic">For <span className="uppercase font-bold">FRESH LINK AGRO COLD STORAGE PVT. LTD.</span></p>
            <div className="pt-8 border-t border-slate-400"><p className="font-bold text-[10px]">Authorised Signatory</p></div>
          </div>
      ...
      <div className="max-w-4xl mx-auto mt-6 flex justify-center gap-4 print:hidden">
        <Button variant="outline" onClick={() => window.print()}><Download className="mr-2 h-4 w-4" /> Download PDF</Button>
        <Button onClick={() => window.print()}><Printer className="mr-2 h-4 w-4" /> Print Delivery Order</Button>
      </div>
